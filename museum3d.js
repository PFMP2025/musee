// Imports CDN sans import map (fiable sur Chrome/Firefox/Safari)
...
function makeLabelTexture(title, subtitle){
  ...
  wrapText(ctx, title, 60, 200, W-120, 54);
...
fetch('./resistantes.json').then(r=>r.json()).then(items => {
  ...
  function addFrame(pos, rotY, data){
    ...
    const frame = new THREE.Mesh(new THREE.BoxGeometry(2.4, 1.8, 0.08),
        new THREE.MeshStandardMaterial({ color:0x22262e, metalness:0.2, roughness:0.3 }));
  ...
  for (let side=0; side<4; side++){
    for (let i=0; i<perSide && idx < n; i++, idx++){
      const d = items[idx];
      let pos, rotY = 0;
      if (side===0){ pos = new THREE.Vector3(-10 + spacing*(i+1), y, -6.6); }
      else if (side===1){ pos = new THREE.Vector3(-10 + spacing*(i+1), y, 6.6); rotY = Math.PI; }
      else if (side===2){ pos = new THREE.Vector3(-9.6, y, -7 + spacingShort*(i+1)); rotY = Math.PI/2; }
      else { pos = new THREE.Vector3(9.6, y, -7 + spacingShort*(i+1)); rotY = -Math.PI/2; }
      addFrame(pos, rotY, d);
    }
  }
});
