// Musée 3D lumineux + images dans les cadres (compat: Chrome/Edge/Firefox/Safari)
...
renderer.physicallyCorrectLights = true;               // <= nécessaire pour RectAreaLight
renderer.toneMapping = THREE.ACESFilmicToneMapping;
renderer.toneMappingExposure = 1.6;                    // exposition plus haute

const scene = new THREE.Scene();
scene.background = new THREE.Color(0x1a2333);          // fond un peu plus clair
...
// --- Salle (murs & sol plus clairs) ---
const room = new THREE.Group();
const floorMat = new THREE.MeshStandardMaterial({ color: 0x2a3a52, roughness: 0.9, metalness: 0.05 });
const wallMat  = new THREE.MeshStandardMaterial({ color: 0x32455c, roughness: 0.95, metalness: 0.0 });
...
// --- Éclairage "musée" ---
scene.add(new THREE.AmbientLight(0xffffff, 0.55));     // base plus lumineuse
const hemi = new THREE.HemisphereLight(0xdfeaff, 0x1a2030, 0.9);
hemi.position.set(0, 8, 0); scene.add(hemi);

// bandes lumineuses rectangulaires au plafond le long des murs
RectAreaLightUniformsLib.init();
function addStripLights(){
  const strips = new THREE.Group();
  const mk = (x,y,z,rx,ry,rz) => {
    const L = new THREE.RectAreaLight(0xffffff, 60, 6, 1); // intensité élevée (physicallyCorrectLights)
    L.position.set(x,y,z); L.rotation.set(rx,ry,rz);
    strips.add(L);
  };
  const y = 4.2;
  // arrière & avant
  mk(-9, y, -8.6, -Math.PI/2.2, 0, 0);
  mk(  0, y, -8.6, -Math.PI/2.2, 0, 0);
  mk(  9, y, -8.6, -Math.PI/2.2, 0, 0);
  mk(-9, y,  8.6, -Math.PI/2.2, Math.PI, 0);
  mk(  0, y,  8.6, -Math.PI/2.2, Math.PI, 0);
  mk(  9, y,  8.6, -Math.PI/2.2, Math.PI, 0);
  // gauche & droite
  mk(-12.6, y, -6, -Math.PI/2.2,  Math.PI/2, 0);
  mk(-12.6, y,  0, -Math.PI/2.2,  Math.PI/2, 0);
  mk(-12.6, y,  6, -Math.PI/2.2,  Math.PI/2, 0);
  mk( 12.6, y, -6, -Math.PI/2.2, -Math.PI/2, 0);
  mk( 12.6, y,  0, -Math.PI/2.2, -Math.PI/2, 0);
  mk( 12.6, y,  6, -Math.PI/2.2, -Math.PI/2, 0);
...
function makeLabelTexture(title, subtitle){
  ...
  g.addColorStop(0,'#1f2c40'); g.addColorStop(1,'#162434');
  ...
  ctx.fillStyle='#bfc8d6'; ctx.font='32px system-ui,Segoe UI,Roboto'; ctx.fillText(subtitle||'', 64, H-80);
...
function makeSmallLabel(title, subtitle){
  ...
  ctx.fillStyle='rgba(18,24,36,0.95)'; ctx.fillRect(0,0,W,H);
...
function makeArtMeshFromImage(url, title, sub){
  return new Promise(resolve=>{
    ...
    texLoader.load(url, (tex)=>{
      ...
      console.warn('Image introuvable / erreur de chargement:', url, err); // <== debug
      const t=makeLabelTexture(title,sub);
      resolve(new THREE.Mesh(new THREE.PlaneGeometry(TOILE_MAX_W,TOILE_MAX_H),
              new THREE.MeshStandardMaterial({ map:t, roughness:0.7 })));
    });
...
function addPictureLight(group){
  // petit spot dédié par tableau (en plus des bandes)
  const spot = new THREE.SpotLight(0xffffff, 20, 6, Math.PI/8, 0.4, 1);
  spot.position.set(0, 1.35, 0.6);
